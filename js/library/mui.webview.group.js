(function(factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        module.exports = factory(require('mui'));
    } else {
        factory(window.mui);
    }
}(function(mui) {
	var m_topDistance = 88;
	if ($app.topDistance) {
		m_topDistance = parseInt($app.topDistance);
	}
	
	var webviewGroup = window.webviewGroup = function(id, options) {
		this.id = id;
	...
	proto._initNativeView = function() {
		this.nativeView = new plus.nativeObj.View('__MUI_TAB_NATIVE', {
			'top': m_topDistance +'px', 
			'height': (window.screen.height - m_topDistance) + "px",
			'left': '100%',
			'width': '100%',
	...
	window.webviewGroup = webviewGroup;
}));
